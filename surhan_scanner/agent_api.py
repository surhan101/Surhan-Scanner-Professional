...
#     if not _has_any_scanner_role(["Surhan Scanner User", "Surhan Scanner Manager", "Surhan Scanner Admin", "System Manager"]):
#         frappe.throw(_("Not permitted"), frappe.PermissionError)


# def _require_scanner_manager_api():
#     _require_logged_in_api()
#     if not _has_any_scanner_role(["Surhan Scanner Manager", "Surhan Scanner Admin", "System Manager"]):
#         frappe.throw(_("Not permitted"), frappe.PermissionError)


# def _require_scanner_admin_api():
#     _require_logged_in_api()
#     if not _has_any_scanner_role(["Surhan Scanner Admin", "System Manager"]):
#         frappe.throw(_("Not permitted"), frappe.PermissionError)
...
# #         frappe.throw(_("Target field must be Attach or Attach Image"))
...
# #     allowed_file_types = allowed_file_types or ["pdf", "jpg", "jpeg", "png", "tif", "tiff"]
...
# #         frappe.throw(_("File content does not match .{0} format").format(extension))
...
# #         allowed_file_types = ["pdf", "jpg", "jpeg", "png", "tif", "tiff"]
...
# #     resolved_file_type = (
# #         file_type or _get_value(settings, "default_file_type", "JPG")
# #     ).lower()
...
# #         resolution or _get_int_value(settings, "default_resolution", 200)
# #     )

# #     resolved_pixel_type = pixel_type or _get_value(
# #         settings,
# #         "default_pixel_type",
# #         "Color"
# #     )
...
# #     resolved_is_private = (
# #         cint(is_private)
# #         if is_private not in [None, ""]
# #         else _get_check_value(settings, "is_private", 1)
# #     )

# #     resolved_folder = folder or _get_value(settings, "folder", "Home/Attachments")
# #     resolved_paper_source = paper_source or "Feeder"

# #     resolved_silent_scan = (
# #         cint(silent_scan)
# #         if silent_scan not in [None, ""]
# #         else _get_check_value(settings, "enable_silent_scan", 1)
# #     )

# #     resolved_show_preview = (
# #         cint(show_preview)
# #         if show_preview not in [None, ""]
# #         else 1
# #     )
...
# #         "doctype": doctype,
# #         "docname": docname,
# #         "attach_field": attach_field or "",
# #         "upload_mode": upload_mode or "Both",
# #         "rule": rule or "",
...
# #         "multi_page": cint(multi_page),
# #         "use_feeder": cint(use_feeder),
# #         "duplex": cint(duplex),
...
# #         "scan_batch_mode": scan_batch_mode or "Single Page",
# #         "max_pages": cint(max_pages or 1),
# #         "upload_strategy": upload_strategy or "Direct Upload",
...
# # def upload_agent_scan(*args, **kwargs):
#     require_scanner_access()
#     rules = get_rules(kwargs.get('scanned_doctype'))
#     return _upload_agent_scan(*args, rules=rules, **kwargs)

# def _upload_agent_scan(*args, **kwargs):
#     require_scanner_access()
#     return _upload_agent_scan(*args, **kwargs)
...
# #                 "Surhan Scanner Uploaded File Read Failed"
# #             )
# #             return _response(400, False, "Could not read uploaded file")
...
# #     allowed_file_types = session_data.get("allowed_file_types") or [
# #         "pdf", "jpg", "jpeg", "png", "tif", "tiff"
# #     ]
...
# #     upload_mode = session_data.get("upload_mode") or "Both"
...
# #         frappe.log_error(
# #             frappe.get_traceback(),
# #             "Surhan Scanner Target Permission Failed"
# #         )
# #         return _response(
# #             403,
# #             False,
# #             "You do not have permission to attach scanned file to this document"
# #         )
...
# #         frappe.log_error(
# #             frappe.get_traceback(),
# #             "Surhan Scanner Save File Failed"
# #         )
# #         return _response(500, False, "Could not save uploaded scan file")

# #     if upload_mode in ["Both", "Set Attach Field"] and attach_field:
# #         try:
# #             target_doc.set(attach_field, file_doc.file_url)
# #             target_doc.save()
# #         except Exception:
# #             frappe.log_error(
# #                 frappe.get_traceback(),
# #                 "Surhan Scanner Attach Field Update Failed"
# #             )
# #             return _response(
# #                 500,
# #                 False,
# #                 "File saved, but could not update target attach field",
# #                 {
# #                     "file": {
# #                         "name": file_doc.name,
...
# #             agent_scanner_name=session_data.get("scanner_name"),
# #             scan_session_id=session_data.get("scan_session_id"),
...
# #         frappe.log_error(
# #             frappe.get_traceback(),
# #             "Surhan Scanner Agent Log Failed"
# #         )
# # -*- coding: utf-8 -*-
...
# # def _sanitize_filename_part(value, max_length=60):
# #     """
# #     ينظف جزءًا من الاسم ليصبح آمنًا داخل اسم الملف.
# #     """
# #     value = str(value or "").strip()
...
# # def _get_employee_label(session_data):
# #     """
# #     يرجع اسم الموظف/المستخدم المستخدم داخل قالب الاسم.
# #     """
# #     raw = (
# #         session_data.get("employee")
# #         or session_data.get("employee_name")
# #         or session_data.get("windows_user")
...
# #     return _sanitize_filename_part(raw, 40)


# # def _get_scan_name_counter_key(doctype, docname, employee_label):
# #     """
# #     مفتاح العدّاد اليومي.
# #     """
# #     date_part = now_datetime().strftime("%Y%m%d")
# #     site = _sanitize_filename_part(frappe.local.site, 40)
# #     doctype_part = _sanitize_filename_part(doctype, 40)
# #     docname_part = _sanitize_filename_part(docname, 60)
# #     employee_part = _sanitize_filename_part(employee_label, 40)

# #     return (
# #         f"surhan_scanner:scan_counter:"
# #         f"{site}:{doctype_part}:{docname_part}:{employee_part}:{date_part}"
# #     )


# # def _get_next_scan_counter(doctype, docname, employee_label):
# #     """
# #     عدّاد يومي متزايد لكل doctype + docname + employee + date.
# #     """
# #     key = _get_scan_name_counter_key(doctype, docname, employee_label)

# #     try:
# #         counter = frappe.cache().incr(key)
# #     except Exception:
# #         counter = cint(frappe.cache().get_value(key) or 0) + 1
# #         frappe.cache().set_value(key, counter, expires_in_sec=172800)
...
# # def _build_default_scan_file_name(session_data, detected_extension):
# #     """
# #     يبني الاسم الافتراضي:
# #     doctype_docname_employee_YYYYMMDD_counter.pdf
# #     """
# #     doctype_part = _sanitize_filename_part(session_data.get("doctype"), 40)
# #     docname_part = _sanitize_filename_part(session_data.get("docname"), 60)
# #     employee_label = _get_employee_label(session_data)
# #     employee_part = _sanitize_filename_part(employee_label, 40)
# #     date_part = now_datetime().strftime("%Y%m%d")
# #     counter = _get_next_scan_counter(
# #         session_data.get("doctype"),
# #         session_data.get("docname"),
# #         employee_label
# #     )
# #     ext = (detected_extension or "pdf").lower().lstrip(".")

# #     return f"{doctype_part}_{docname_part}_{employee_part}_{date_part}_{counter:03d}.{ext}"


# # def _resolve_final_scan_file_name(session_data, detected_extension):
# #     """
# #     يحدد الاسم النهائي:
# #     - custom_file_name إذا كان موجودًا ومسموحًا
# #     - أو الاسم الافتراضي التلقائي
# #     """
# #     custom_file_name = session_data.get("custom_file_name")

# #     if custom_file_name:
# #         if not _user_has_scanner_manager_role(frappe.session.user):
# #             frappe.throw(_("Only managers can set a custom file name"))

# #         custom_file_name = _normalize_custom_scan_filename(custom_file_name)
# #         if not custom_file_name:
# #             frappe.throw(_("Custom file name is required"))

# #         custom_base = os.path.basename(custom_file_name)
# #         base_name, _ = os.path.splitext(custom_base)
# #         base_name = _sanitize_filename_part(base_name, 120)
# #         ext = (detected_extension or "pdf").lower().lstrip(".")
# #         return f"{base_name}.{ext}"

# #     return _build_default_scan_file_name(session_data, detected_extension)
# #     custom_file_name = _normalize_custom_scan_filename(custom_file_name)
# #     if not custom_file_name:
# #         return original_filename
...
# #             final_file_name = _resolve_final_scan_file_name(session_data, detected_extension)
# #         except Exception as exc:
# #             return _response(
# #                 400,
# #                 False,
...
# #                 target_doc.save()
# #             except Exception:
# #                 frappe.log_error(frappe.get_traceback(), "Surhan Scanner Attach Field Update Failed")
# #                 return _response(
# #                     500,
# #                     False,
# #                     "File saved, but could not update target attach field",
